<<<<<<< HEAD
var LivePersonVirtualAssistantModule=(function(){var _version='2.0.9';var _config={USING_PROXY_BUTTON:!0,TRIGGER_CHAT_BUTTON_FROM_BUSY_STATE:!1,TRIGGER_CHAT_BUTTON_FROM_OFFLINE_STATE:!1,SEND_FAQ_CONVERSATION_AS_PRECHAT_LINES:!0,PRECHAT_LINES_INTRO_MESSAGE_ENABLED:!0,EMBEDDED_BUTTON_ID_LOADED:null,EMBEDDED_BUTTON_TYPE:5,EMBEDDED_BUTTON_INFO:null,ENGAGEMENT_NAME_SHORTCODE:':vap:',EMBEDDED_BUTTON_DIV_CONTAINER_ID:'lpButtonDiv-need-help-panel',WINDOW_CLOSE_BUTTON_CLASS:'lp_close',NAMESPACE:'LP_VA_PANEL_MODULE',EVENTS:{BUTTON_IMPRESSION:'EMBEDDED_BUTTON_IMPRESSION',SHOW:'SHOULD_SHOW_VA_PANEL',HIDE:'SHOULD_HIDE_VA_PANEL',BUTTON_TO_DISPLAY:'SHOULD_SHOW_BUTTON_CONTENT',CLOSE_THANKYOU_WINDOW:'DID_CLOSE_THANKYOU_WINDOW'}};var _supportedLanguages=['en','fr','zh_hans','zh','zh_tw','ar','bm','es','es_mx'];var _abandonedChatEvents=['waiting','preChat','chatting','postChat'];var _translations={'intro':{'en':'en - Your conversation history so far...','fr':'fr - ???...','zh':'zh_hans - ???...','zh_tw':'zh_hant - ???...','ar':'ar - ???...','bm':'bm - ???...','es':'es - ???...','es_mx':'es_mx - ???...','zh_hans':'zh_hans - ???...'},'suffix':{'en':'en - An Agent will be with you shortly...','fr':'fr - ???...','zh':'zh_hans - ???...','zh_tw':'zh_hant - ???...','ar':'ar - ???...','bm':'bm - ???...','es':'es - ???...','es_mx':'es_mx - ???...','zh_hans':'zh_hans - ???...'}};var BUTTON_STATES={'ONLINE':1,'OFFLINE':2,'BUSY':4,'UNKNOWN':0};var BUTTON_STATE_DESCRIPTIONS={1:'ONLINE',2:'OFFLINE',4:'BUSY',0:'UNKNOWN',};var _eventBindingsDone=!1;var _eventLog=[];function _log(name,event){_eventLog.push({'name':name,'info':event,'timestamp':Date.now()})}
=======
var LivePersonVirtualAssistantModule=(function(){var _version='2.1.0';var _config={COUNTRY_CODE_LOCATION:'cstatus',USING_PROXY_BUTTON:!0,TRIGGER_CHAT_BUTTON_FROM_BUSY_STATE:!1,TRIGGER_CHAT_BUTTON_FROM_OFFLINE_STATE:!1,SEND_FAQ_CONVERSATION_AS_PRECHAT_LINES:!0,PRECHAT_LINES_INTRO_MESSAGE_ENABLED:!0,EMBEDDED_BUTTON_ID_LOADED:null,EMBEDDED_BUTTON_TYPE:5,EMBEDDED_BUTTON_INFO:null,ENGAGEMENT_NAME_SHORTCODE:':vap:',EMBEDDED_BUTTON_DIV_CONTAINER_ID:'lpButtonDiv-need-help-panel',WINDOW_CLOSE_BUTTON_CLASS:'lp_close',NAMESPACE:'LP_VA_PANEL_MODULE',EVENTS:{BUTTON_IMPRESSION:'EMBEDDED_BUTTON_IMPRESSION',SHOW:'SHOULD_SHOW_VA_PANEL',HIDE:'SHOULD_HIDE_VA_PANEL',BUTTON_TO_DISPLAY:'SHOULD_SHOW_BUTTON_CONTENT',CLOSE_THANKYOU_WINDOW:'DID_CLOSE_THANKYOU_WINDOW'}};var _supportedLanguages=['en','fr','zh_hans','zh','zh_cn','zh_tw','ar','bm','es','es_mx'];var _abandonedChatEvents=['waiting','preChat','chatting','postChat'];var _translations={'default':{'intro':'Your conversation history so far ...','suffix':'An Agent will be with you shortly ...'},'en':{'intro':'Your conversation history so far ...','suffix':'An Agent will be with you shortly ...'},'es_mx':{'intro':'Su historial de conversaciones hasta la fecha...','suffix':''},'fr':{'intro':'Votre historique de conversations jusqu\'à présent...','suffix':'Un agent vous répondra bientôt...'},'zh':{'intro':'很快将会有座席代表与您聊天...','suffix':'很快将会有座席代表与您聊天...'},'zh_cn':{'intro':'很快将会有座席代表与您聊天...','suffix':'很快将会有座席代表与您聊天...'},'zh_tw':{'intro':'您目前的對談記錄...','suffix':'服務人員很快會與您聯絡...'},'zh_hans':{'intro':'','suffix':'Dentro de poco un agente estará con usted...'},'ar':{'intro':'','suffix':''},'bm':{'intro':'','suffix':''},'es':{'intro':'','suffix':''}};var BUTTON_STATES={'ONLINE':1,'OFFLINE':2,'BUSY':4,'UNKNOWN':0};var BUTTON_STATE_DESCRIPTIONS={1:'ONLINE',2:'OFFLINE',4:'BUSY',0:'UNKNOWN',};var _eventBindingsDone=!1;var _eventLog=[];function _log(name,event){_eventLog.push({'name':name,'info':event,'timestamp':Date.now()})}
>>>>>>> hsbcnet/develop
...
<<<<<<< HEAD
function triggerChatButtonClick(faqHistorySoFar){var clicked;if(lpTag&&lpTag.taglets&&lpTag.taglets.rendererStub){if(_config.SEND_FAQ_CONVERSATION_AS_PRECHAT_LINES&&faqHistorySoFar.length>0){var preChatLinesIntroMessages=getPreChatLinesIntroMessages();var preChatLinesArray=[];if(preChatLinesIntroMessages){preChatLinesArray.push(preChatLinesIntroMessages)}else if(preChatLinesIntroMessages===!1){}
preChatLinesArray=preChatLinesArray.concat(faqHistorySoFar);var preChatLinesSuffixMessage=getPreChatLinesSuffixMessages();if(preChatLinesSuffixMessage){preChatLinesArray.push(preChatLinesSuffixMessage)}
...
function getCurrentLanguageSelection(){var cartItems=lpTag.sdes.get('cart')[0]||!1;console.log('==> cartItems // ',cartItems);var foundSupportedLanguage=!1;var currentLanguageSelection=null;if(cartItems&&cartItems.products&&cartItems.products.length){for(let i=0;i<cartItems.products.length;i++){const possibleLanguageSelection=cartItems.products[i].product.name||null;if(isSupportedLanguage(possibleLanguageSelection)){currentLanguageSelection=possibleLanguageSelection;foundSupportedLanguage=!0;break}}}
return currentLanguageSelection}
function getPreChatLinesIntroTranslation(){var language=getCurrentLanguageSelection();if(language){return _translations.intro[language]}else{return!1}}
function getPreChatLinesSuffixTranslation(){var language=getCurrentLanguageSelection();if(language){return _translations.suffix[language]}else{return!1}}
function getPreChatLinesIntroMessages(){var preChatLinesIntroMessage=getPreChatLinesIntroTranslation();return preChatLinesIntroMessage||!1}
function getPreChatLinesSuffixMessages(){var preChatLinesSuffixMessage=getPreChatLinesSuffixTranslation();return preChatLinesSuffixMessage||!1}
=======
...
function getCountrySelection(){var customerInfoExists=lpTag.sdes.get('ctmrinfo')[0]||null;var countryCode=null;if(customerInfoExists&&customerInfoExists.info){var customerInfo=customerInfoExists.info;countryCode=customerInfo[_config.COUNTRY_CODE_LOCATION]||null}
return countryCode}
function getCurrentLanguageSelection(){var cartItems=lpTag.sdes.get('cart')[0]||!1;console.log('[LP VA Module] ==> getCurrentLanguageSelection : cartItems // ',cartItems);var foundSupportedLanguage=!1;var currentLanguageSelection=null;if(cartItems&&cartItems.products&&cartItems.products.length){for(let i=0;i<cartItems.products.length;i++){var possibleLanguageSelection=cartItems.products[i].product.name||null;if(isSupportedLanguage(possibleLanguageSelection)){currentLanguageSelection=possibleLanguageSelection;foundSupportedLanguage=!0;console.log('[LP VA Module] ==> getCurrentLanguageSelection : foundSupportedLanguage // ',currentLanguageSelection);break}}}
return currentLanguageSelection}
